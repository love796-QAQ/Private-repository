        ...
        switch (type) {
          ...
          case "cronexp=":
            ...
            if (isLooniOS) {
              ...
              if (x.match(/cronexp=(.+?),[^,]+?=/)) {
                script.push(
                  x.replace(/(.+cronexp=).+?(,[^,]+?=.*)/, `$1"${cronExp}"$2`).replace(/"{2,}/g, '"'));
              } else {
                script.push(
          ...
          case " data=":
            ...
            if (isSurgeiOS) {
              ...
              if (isLooniOS || isShadowrocket) {
                ...
                URLRewrite.push(
                  `${noteK4}- >-${noteKn6}${ptn} - reject${mock2Reject}`);
              } else {
          ...
          default:
            ...
            if (type.match(/ (302|307|header)/)) {
              ...
              if (isLooniOS) {
                ...
                rules.push(x.replace(/^#?DEST-PORT *,/, `${noteK}DST-PORT,`));
              } else { rules.push(x); }//Loon Surge 火箭 rule处理完毕
